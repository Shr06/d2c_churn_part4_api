...
# FASTAPI APP
app = FastAPI(title="D2C Customer Churn Prediction API",version="1.0")
# RISK LOGIC
...
def generate_risk_explanation(customer, probability):
  ...
  if probability >= 0.70:
    return ("Customer shows several patterns associated with churn risk.")
    return ("Customer engagement indicators appear healthy.")
...
def batch_predict(batch: BatchCustomerData):
  try:
    ...
    results = []
    for customer, prob in zip(batch.customers,probabilities):
      results.append({"churn_probability": round(float(prob), 4),
        "predicted_class": int(prob >= CHURN_THRESHOLD),
        "risk_level": get_risk_level(float(prob)),
        "risk_explanation":generate_risk_explanation(customer,float(prob))
                })
